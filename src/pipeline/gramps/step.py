...
about:
  ...
  - hand-cropped portraits from data/gramps/portraits/, attached whole (they are
    already cropped to the face) and referenced where they lie;
...
dropping a correctly-named file into data/gramps/portraits/ is enough to give
someone a portrait, no annotation needed. Because the media points straight at
that file, replacing it on disk changes the portrait with no re-run at all. See
pipeline.gramps.plan for the ordering rules and pipeline.gramps.media for why
the raw photos (and only they) are baked upright first.

Re-running is safe and additive: media the export already points at the same
file is reused rather than duplicated, and a reference a person already carries
is not added twice.
...
from pipeline.gramps.media import MEDIA_DIR, bake_upright
...
def _bake_media(plan: Plan, config: GrampsConfig) -> tuple[int, int, int, int]:
    """Bake an upright copy of every planned image that needs one.

    Hand-cropped portraits are pointed at in place and never baked, so this only
    walks the raw photos. Returns (baked, skipped, missing, failed). A missing
    source is reported but not fatal: the media object is still written, so the
    export tells you which file to restore.
    """
    state = state_lib.load(STATE_FILE)
    baked = skipped = missing = failed = 0

    for item in plan.media.values():
        if item.bake_from is None:
            continue
        ...
        if not config.ignore_state and state_lib.is_done(state, item.key):
            skipped += 1
            continue
        if not item.bake_from.exists():
            logger.warning(f"Source image missing: {rel(item.bake_from)}")
            missing += 1
            continue

        try:
            bake_upright(item.bake_from, item.path, config.jpeg_quality)
            state_lib.mark_done(state, item.key, [str(item.path)])
            ...
            state_lib.save(state, STATE_FILE)
            logger.error(f"Failed to bake {rel(item.bake_from)}: {e}")
            failed += 1

    return baked, skipped, missing, failed


def _merge(plan: Plan, export: GrampsExport, config: GrampsConfig) -> tuple[int, int, int]:
    """Apply the plan to the export. Returns (media added, refs added, pruned)."""
    if config.replace_media:
        export.clear_person_media(set(plan.refs_by_person))

    # Media first: it resolves each key to a handle, reusing any object the
    # export already points at the same file, which the refs then follow.
    ...
    return len(added), refs_added, pruned
